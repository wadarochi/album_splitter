...
Split single-file audio rips (FLAC, WAV, APE) into individual tracks using format-specific backends with auto-detection, lossless guarantee, and streaming I/O.

## Requirements

### Requirement: Format-specific splitting backends
The system SHALL select the splitting backend based on input audio format (detected from file extension): FLAC uses flac-tracksplit → flacsplt → shntool fallback chain; WAV uses Python wave module; APE uses ffmpeg decode (primary) → mac.exe fallback → WAV split → optional FLAC re-encode.
...
#### Scenario: APE splitting uses ffmpeg first, mac.exe fallback
- **WHEN** splitting an APE file
- **THEN** the system SHALL decode APE to temporary WAV using ffmpeg (primary) or mac.exe (fallback), split the WAV, then optionally re-encode each track to FLAC
...
- **WHEN** decoding APE to WAV using ffmpeg or mac.exe
